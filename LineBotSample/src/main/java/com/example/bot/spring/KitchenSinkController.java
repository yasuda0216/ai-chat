...
import com.example.bot.spring.replier.Follow;
...
public class KitchenSinkController {
    ...
    private void replyTexts(@NonNull String replyToken, @NonNull String[] message) {
        if (replyToken.isEmpty()) {
            throw new IllegalArgumentException("replyToken must not be empty");
        }
        List<Message> list = new ArrayList<Message>();
        for (int i = 0; i < message.length; i++) {
        	list.add(new TextMessage(message[i]));
        }
    ...
    public TextMessage handleFollowEvent(FollowEvent event ) {
        ...
        return new TextMessage("友達追加ありがとう！！");
    }
